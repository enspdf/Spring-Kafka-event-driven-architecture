package me.shackox.productorderanalytics.processor;
...
public class ProductOrdersProcessor {
    ...
    public void sink(ProductOrdersEvent productOrdersEvent) {
        LOGGER.info("New ProductOrderEvent {} - {} - {}", productOrdersEvent.getName(),
                productOrdersEvent.getCategory(), productOrdersEvent.getQuantity());
    }
    ...
    public void processAnalytics(KStream<String, ProductOrdersEvent> input) {
        input.map((key, value) -> new KeyValue<>(value.getCategory(), 0l))
                ...
                .count(Materialized.as(ProductOrdersBindings.PRODUCT_ORDERS_CATEGORY_COUNT)).toStream()
